...
public class MainMenu : MonoBehaviour
{
    ...
    {
        SceneManager.LoadScene(3); // Load Controller Scene

        //        if (currentFloor == floorCount) currentFloor = 0; // Check if level exists
        //        if (!dead) SetCurrentFloor(currentFloor + 1);

        yield return SceneManager.LoadSceneAsync(currentLevel + 4, LoadSceneMode.Additive); // Level is 0, but scene's in build start at 2 for levels

        SceneManager.SetActiveScene(SceneManager.GetSceneByBuildIndex(currentLevel + 4)); // This allows the Navmesh to work for the AI
        //        SpawnPlayer();
    }
